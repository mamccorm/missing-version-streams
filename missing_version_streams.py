...
def main(package_repo, version_streams_repo, endoflife_repo):
    ...
    matched_files = [file for file in package_files_with_numbers if file.split('-')[0] in version_stream_files]
    unmatched_files = [file for file in package_files_with_numbers if file.split('-')[0] not in version_stream_files]
    endoflife_matches_unmatched = [file for file in unmatched_files if file.split('-')[0] in endoflife_files]
    endoflife_matches_no_version = [file for file in package_files_without_numbers if file.split('.')[0] in endoflife_files]
    ...
    print("\nSummary:")
    print(f"  Total packages found with numbers: {total_packages_with_numbers}")
    print(f"  Total with version streams: {total_with_streams}")
    print(f"  Versioned packages in wolfi missing a version stream: {total_without_streams}")
    print(f"  Total end-of-life matches (unmatched): {total_endoflife_matches_unmatched}")
    print(f"  Total packages without version numbers: {total_packages_without_numbers}")
    print(f"  Total end-of-life matches (no version number): {total_endoflife_matches_no_version}")
    ...
    with open('endoflife-versioned-but-missing-in-wolfi.txt', 'w') as f:
        all_endoflife_matches = sorted(endoflife_matches_unmatched + endoflife_matches_no_version)
        filtered_endoflife_matches = [file for file in all_endoflife_matches if file.split('-')[0] not in version_stream_files]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Find and match package files with version streams and end-of-life information")
    parser.add_argument('--package-repo', required=True, help='Path to the package repository')
    parser.add_argument('--version-streams-repo', required=True, help='Path to the version streams repository')
    parser.add_argument('--endoflife-repo', required=True, help='Path to the end-of-life repository')
